...
import { runsStore, type Run } from "@/lib/runs-store";
...
export function WorkflowTimeline({ run }: { run: Run }) {
  ...
  useEffect(() => {
    ...
    if (stageIdx >= STAGES.length) {
      runsStore.update(run.id, { status: "complete", stage: "validate" });
      return;
    }
    const t = setTimeout(() => {
      const next = stageIdx + 1;
      setStageIdx(next);
      const stage = STAGES[Math.min(next, STAGES.length - 1)].key;
      runsStore.update(run.id, next >= STAGES.length ? { status: "complete", stage } : { stage });
    }, 900);
    return () => clearTimeout(t);
  }, [stageIdx, run.id, run.status]);

  const complete = run.status === "complete" || stageIdx >= STAGES.length;
